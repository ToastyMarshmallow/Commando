...
class Command {
	...
	onBlock(message, reason, data) {
		switch(reason) {
			...
			case 'throttling': {
				let sec_num = parseInt(data.remaining.toFixed(1), 10);
				...
				let howlongago = `${hours > 0 ? `${hours} hours ` : ""}${minutes > 0 ? `${minutes} minutes ` : ""}${seconds > 0 ? `${seconds} seconds` : ""}`

					return message.reply(
					`You may not use the \`${this.name}\` command again for another ${howlongago}.`
				);
			}
			default:
				return null;
